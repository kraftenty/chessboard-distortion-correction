...
if __name__ == '__main__':
    ...
    print(f'* RMS error = {rms:.6f}')  # 소수점 이하 6자리까지 표현

    # 카메라 매트릭스(K) 출력
    K_str = '\n'.join([' '.join([f'{item:.3f}' for item in row]) for row in K])
    print(f'* Camera matrix (K) = \n{K_str}')

    # 왜곡 계수(dist_coeff) 출력
    dist_coeff_str = ' '.join([f'{item:.6f}' for item in dist_coeff.flatten()])
    print(f'* Distortion coefficient (k1, k2, p1, p2, k3, ...) = {dist_coeff_str}')
